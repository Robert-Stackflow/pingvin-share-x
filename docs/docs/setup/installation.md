...
The website is now listening on `http://localhost:3000`, have fun with Share!
...
Your container is now listening on `http://localhost:<externalport>`, have fun with Share!
...
- [pm2](https://pm2.keymetrics.io/) for running Share in the background
...
**Uploading Large Files**: By default, Share uses a built-in reverse proxy to reduce the installation steps. However, this reverse proxy is not optimized for uploading large files. If you wish to upload larger files, you can either use the Docker installation or set up your own reverse proxy. An example configuration for Caddy can be found in `./reverse-proxy/Caddyfile`.
